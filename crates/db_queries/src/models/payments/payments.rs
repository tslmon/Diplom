...
use db_schema::schema::{payments, payments::dsl::*};
...
impl Crud<PaymentForm, PaymentId> for Payment {
    fn create(_conn: &PgConnection, _form: &PaymentForm) -> Result<Self, ModelError> {
        let _result = insert_into(payments)
            .values(_form)
    ...
    fn read(_conn: &PgConnection, _id: &PaymentId) -> Result<Self, ModelError> {
        let _result = payments.find(_id).first::<Self>(_conn);
    ...
    fn update(
        ...
        let _result = diesel::update(payments.find(_id))
            .set(_form)
            .get_result::<Self>(_conn);
        match _result {
            Ok(res) => Ok(res),
    ...
    fn delete(_conn: &PgConnection, _id: &PaymentId) -> Result<usize, ModelError> {
        let _result = diesel::delete(payments.find(_id)).execute(_conn);
...
impl ManagementAsyncTrait<PaymentForm, PaymentId> for Payment {
        ...
        Self: Sized,
    {
        ...
        let mut query = match (_q, _sort) {
            (Some(q), Some(sort)) => {
                ...
                Self::payments_by_filter(&s)
            }
            (Some(q), None) => Self::payments_by_filter(q),
            (None, Some(sort)) => {
                s.push_str(" 1=1 ORDER BY ");
                s.push_str(&sort.join(", "));
                Self::payments_by_filter(&s)
            }
            (None, None) => payments.into_boxed(),
        };
        if let Some(total_count) = _total_count {
            if *total_count {
                let _total_sql = match _q {
                    Some(q) => Self::payments_by_filter(q),
                    None => payments.into_boxed(),
                };
        ...
        let mut _result = query.load::<Payment>(_conn);
    ...
    where
        Self: Sized,
    {
        Payment::update(_conn, _id, _form)
    }

    async fn delete_item(_conn: &PgConnection, _id: &PaymentId) -> Result<usize, ModelError> {
        Payment::delete(_conn, _id)
    }
}
...
pub trait Payment_ {
    /// Helper functions
    fn payments_by_filter<'a>(_sql: &'a str) -> payments::BoxedQuery<'a, diesel::pg::Pg> {
        payments::table.filter(sql(_sql)).into_boxed()
    }
}
